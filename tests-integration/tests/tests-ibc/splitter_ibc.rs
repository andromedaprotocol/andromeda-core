...
fn run_splitter_test_on_multiple_combos() {
    ...
    for(chain1, chain2) in &chain_combos.chains {
        let splitter = SplitterContract::new(chain1.chain.clone());
        splitter.upload().unwrap();
    println!("Running test for chain1: {} and chain2: {}", chain1.chain_name, chain2.chain_name);
        // Initialize with percentage splits
    splitter
    .instantiate(
        &InstantiateMsg {
            recipients: vec![
    ...
    chain1
        ...
        .execute(
            &os::adodb::ExecuteMsg::Publish {
                code_id: splitter.code_id().unwrap(),
