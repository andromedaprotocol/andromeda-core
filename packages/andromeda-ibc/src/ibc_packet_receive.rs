...
use crate::{
    ...
    ics721::{
        CallbackMsg, Class, ClassId, ExecuteMsg, NonFungibleTokenPacketData, Token, TokenId,
        VoucherCreation, VoucherRedemption, INCOMING_CLASS_TOKEN_TO_CHANNEL,
...
pub(crate) fn receive_ibc_packet(
    ...
    let submessage = data
        .token_ids
        .iter()
        .zip_optional(data.clone().token_uris)
        .zip_optional(data.clone().token_data)
        .try_fold(
