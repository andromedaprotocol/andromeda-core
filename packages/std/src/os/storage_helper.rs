...
impl StorageHelper {
    ...
    pub fn verify_code_id(
        ...
        let key =
            StorageHelper::get_map_storage_key("ado_type", &vec![code_id.to_string().as_bytes()])?;
        let verify: Option<String> = StorageHelper::query_storage(&querier, &adodb_addr, &key)?;
    ...
    pub fn code_id_getter(
        ...
        let key = StorageHelper::get_map_storage_key("code_id", &vec![ado_type.as_bytes()])?;
        let verify: Option<u64> = StorageHelper::query_storage(&querier, &adodb_addr, &key)?;
    ...
    pub fn vfs_address_getter(
        ...
        let key =
            StorageHelper::get_map_storage_key("kernel_addresses", &vec![VFS_KEY.as_bytes()])?;
        let verify: Option<Addr> = StorageHelper::query_storage(querier, &kernel_addr, &key)?;
        match verify {
