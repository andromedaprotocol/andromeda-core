...
use cosmwasm_std::{
    attr, ensure, to_json_binary, Addr, Binary, Deps, DepsMut, Env, MessageInfo, Reply, Response,
    StdError,
};
use cw721::{
    execute::{
        approve, approve_all, burn_nft, instantiate as cw721_instantiate, mint, revoke, revoke_all,
        send_nft, transfer_nft,
    },
    msg::Cw721InstantiateMsg,
    query::{
        query_all_nft_info, query_all_tokens, query_approval, query_approvals,
        query_collection_info, query_nft_info, query_num_tokens, query_operators, query_owner_of,
        query_tokens,
    },
};
...
fn execute_batch_mint(
    ...
    for msg in tokens_to_mint {
        ...
        let owner = msg.owner.get_raw_address(&ctx.deps.as_ref())?.into_string();
        let mint_resp = mint(
            ctx.deps,
            &ctx.env,
            &ctx.info,
