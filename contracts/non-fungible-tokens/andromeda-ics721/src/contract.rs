...
const CONTRACT_NAME: &str = "crates.io:ics721-bridge";
const CONTRACT_VERSION: &str = env!("CARGO_PKG_VERSION");

const INSTANTIATE_CW721_REPLY_ID: u64 = 0;
/// Submessage reply ID used for instantiating the proxy contract.
const INSTANTIATE_PROXY_REPLY_ID: u64 = 1;
...
pub fn instantiate(
    ...
    let proxy_instantiate = msg
        .proxy
        .map(|m| m.into_wasm_msg(env.clone().contract.address))
    ...
    ADOContract::default()
        .instantiate(
            ...
            info,
            BaseInstantiateMsg {
                ado_type: "ics721-bridge".to_string(),
...
fn execute_handle_packet_receive(
    ...
    let mut messages: Vec<WasmMsg> = Vec::with_capacity(2);
...
fn execute_mint(
    ...
    receiver: String,
        ...
        token_ids.len() == token_uris.len(),
        ContractError::ImbalancedTokenInfo {}
    );
