...
use andromeda_protocol::modules::hooks::{ ATTR_PAYMENT, ATTR_DESC};
...
use andromeda_protocol::receipt::{InstantiateMsg as ReceiptInstantiateMsg,
     ExecuteMsg as ReceiptExecuteMsg, QueryMsg as  ReceiptQueryMsg, ReceiptResponse, Receipt};

#[cfg(not(feature = "library"))]
use cosmwasm_std::entry_point;

use cosmwasm_std::{
    coin, from_binary, to_binary, Addr, Api, Binary, Deps, DepsMut, Env, MessageInfo, Order,
    Pair, Response, StdError, StdResult, QueryRequest, WasmQuery, WasmMsg, SubMsg, ReplyOn, Reply,
    Uint128, CosmosMsg,
};
use cw721::{
...
use crate::state::{
    has_transfer_rights, increment_num_tokens, store_config, read_config, TokenConfig, CONFIG, NUM_TOKENS, OPERATOR, TOKENS,
};
...
pub fn reply(deps: DepsMut, _env: Env, msg: Reply) -> StdResult<Response> {
    match msg.id {
        1 => { // callback after creating receipt contract.
...
fn query_receipt_info(deps: Deps, receipt_id: Uint128) -> StdResult<ReceiptResponse>{
    let config = read_config(deps.storage)?;
    let receipt_response = deps.querier.query(
        &QueryRequest::Wasm(
            WasmQuery::Smart {
                contract_addr: config.receipt_addr,
...
mod tests {
    ...
    fn test_transfer() {
        ...
        //store config
        store_config(
            deps.as_mut().storage, &TokenConfig{
                name: TOKEN_NAME.to_string(),
                symbol: TOKEN_SYMBOL.to_string(),
                minter: String::from("creator"),
