...
fn test_lock_and_mint_factory_success() {
    ...
    assert!(res
        .attributes
        .iter()
        .any(|attr| attr.key == "action" && attr.value == "lock_and_mint_factory"));
    assert!(res
        .attributes
        .iter()
        .any(|attr| attr.key == "cw20_tokens_locked" && attr.value == "100"));
...
fn test_transfer_with_locked_tokens() {
    ...
    LOCKED_TOKENS
        .save(deps.as_mut().storage, &locked_info)
        .unwrap();
...
fn test_unlock_from_factory_success() {
    ...
    LOCKED_TOKENS
        ...
        .unwrap();
    BALANCES
        .save(deps.as_mut().storage, &user, &Uint128::from(900u128))
        .unwrap();
    ...
    assert!(res
        .attributes
        .iter()
        .any(|attr| attr.key == "action" && attr.value == "unlock_from_factory"));
    assert!(res
        .attributes
        .iter()
        .any(|attr| attr.key == "amount_unlocked" && attr.value == "50"));
...
fn test_unlock_from_factory_full_unlock() {
    ...
    assert!(res
        .attributes
        .iter()
        .any(|attr| attr.key == "amount_unlocked" && attr.value == "100"));
