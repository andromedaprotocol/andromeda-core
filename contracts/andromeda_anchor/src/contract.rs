...
pub fn execute_deposit( deps: DepsMut, env: Env,  info: MessageInfo ) -> Result<Response, ContractError> {
    ...
    let aust_balance = query_token_balance(&deps.querier, deps.api.addr_humanize(&config.anchor_token)?, env.contract.address)?;
    ...
    Ok(Response::new().add_messages(vec![
        ...
        CosmosMsg::Wasm( WasmMsg::Execute {
            ...
            msg: to_binary( &ExecuteMsg::Yourself{
                yourself_msg: YourselfMsg::TransferUst { receiver: deps.api.addr_humanize(&position.owner)?.to_string() }
            })?,
            funds: vec![]
        })
    ]
...
pub fn reply(deps: DepsMut, env: Env, msg: Reply) -> StdResult<Response> {
    match msg.id {
        1u64 =>{ // stores aUST amount to position
            let config = CONFIG.load(deps.storage)?;
