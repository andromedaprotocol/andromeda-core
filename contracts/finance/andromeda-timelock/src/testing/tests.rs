...
fn test_execute_hold_funds() {
    ...
    let condition = EscrowCondition::Expiration(MillisecondsExpiration::from_seconds(
...
fn test_execute_hold_funds_escrow_updated() {
    ...
    let msg = ExecuteMsg::HoldFunds {
        condition: Some(EscrowCondition::Expiration(
            MillisecondsExpiration::from_seconds(env.block.time.seconds() + 1),
        )),
    ...
    let expected = Escrow {
        ...
        condition: Some(EscrowCondition::Expiration(
            MillisecondsExpiration::from_seconds(env.block.time.seconds()),
        )),
...
fn test_execute_release_funds_block_condition() {
    ...
    let msg = ExecuteMsg::HoldFunds {
        condition: Some(EscrowCondition::Expiration(
            MillisecondsExpiration::from_nanos(env.block.time.nanos()),
        )),
        recipient: None,
    };
...
fn test_execute_release_funds_time_condition() {
    ...
    let msg = ExecuteMsg::HoldFunds {
        condition: Some(EscrowCondition::Expiration(
            MillisecondsExpiration::from_seconds(100),
        )),
        recipient: None,
    };
