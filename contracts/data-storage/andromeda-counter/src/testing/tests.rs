...
use super::mock::{
    ...
    reset, set_decrease_amount, set_increase_amount, update_restriction,
};
...
fn test_instantiation_private() {
    ...
    let decrease_amount = query_decrease_amount(deps.as_ref()).unwrap().decrease_amount;
...
fn test_update_restriction() {
    ...
    let restriction = query_restriction(deps.as_ref()).unwrap().restriction;
    assert_eq!(restriction, CounterRestriction::Public);
    update_restriction(deps.as_mut(), CounterRestriction::Private, info.sender.as_ref()).unwrap();
    let restriction = query_restriction(deps.as_ref()).unwrap().restriction;
    assert_eq!(restriction, CounterRestriction::Private);
}
